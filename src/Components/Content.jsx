...
function Content() {
    const apiKey = "d4d21c73db57739b6a2f6561dbf6e104"
    ...
    for(let i = 0; i<9; i++){
        let next = first + i;
        let nextday = (new Date(curr.setDate(next))).toString().slice(0,3);
        console.log(nextday)
     ...
     useEffect(() => {
        ...
        const api_call = await fetch(
            `https://api.openweathermap.org/data/2.5/onecall?lat=${city.lat}&lon=${city.long}&units=metric&exclude=current,minutely,hourly,alerts&appid=${apiKey}`
         );
